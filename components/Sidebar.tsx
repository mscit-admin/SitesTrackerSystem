...
const NAV: { href: string; key: string; icon: LucideIcon; perm?: string }[] = [
  { href: "/", key: "nav.dashboard", icon: LayoutDashboard, perm: "dashboard.view" },
  { href: "/acquisition", key: "nav.acquisition", icon: Compass, perm: "acquisition.view" },
  { href: "/sites", key: "nav.sites", icon: Antenna, perm: "sites.view" },
  { href: "/maintenance", key: "nav.maintenance", icon: Wrench, perm: "maintenance.view" },
  { href: "/risks", key: "nav.risks", icon: ShieldAlert, perm: "risks.view" },
  { href: "/deletions", key: "nav.deletions", icon: Trash2, perm: "deletions.view" },
  { href: "/users", key: "nav.users", icon: Users, perm: "users.view" },
  { href: "/settings", key: "nav.settings", icon: Settings, perm: "settings.view" },
];
    ...
    <aside className="sticky top-0 z-20 flex h-screen w-16 shrink-0 flex-col border-l border-gray-200 bg-white md:w-60">
      ...
      <div className="flex h-[52px] items-center gap-2.5 border-b border-gray-200 px-3 md:px-4">
        ...
        <div className="hidden md:block">
          ...
          <div className="text-[11px] text-gray-500">{t("app.subtitle")}</div>
        </div>
      </div>
        ...
        {nav.map((n) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="hidden md:inline">{t(n.key)}</span>
            </Link>
          );
        })}
      </nav>
      ...
      <div className="border-t border-gray-200 p-3 text-[11px] leading-relaxed text-gray-400">
        <div className="hidden md:block">{t("app.company")}</div>
        <div className="hidden md:block">{t("app.network")}</div>
      </div>
    </aside>
  );
}
